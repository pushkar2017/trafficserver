...
public:
  ...
  virtual void attach_server_session(HttpServerSession *s) = 0;
  ...
  virtual void do_hostdb_update_if_necessary() = 0;
  ...
  virtual void dump_state_on_assert() = 0;
  ...
  virtual bool is_redirect_required();
  ...
  virtual int populate_client_protocol(std::string_view *result, int n) const;
  virtual const char *client_protocol_contains(std::string_view tag_prefix) const;
  virtual std::string_view find_proto_string(HTTPVersion version) const;
...
protected:
  ...
  BasicSMHandler default_handler = nullptr;
  Action *pending_action        = nullptr;
  Continuation *schedule_cont   = nullptr;

  virtual void start_sub_sm() = 0;
  ...
  int state_common_wait_for_transform_read(HttpTransformInfo *t_info, BasicSMHandler tunnel_handler, int event, void *data);
  ...
  void setup_internal_transfer(BasicSMHandler handler);
  ...
  int (BasicSM::*m_last_state)(int event, void *data) = nullptr;
