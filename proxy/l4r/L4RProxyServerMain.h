...
// We can use HttpProxyPort as there is nothing HTTP specific defined in it
struct HttpProxyPort;

/// Perform any pre-thread start initialization.
void prep_L4RProxyServer();

/** Initialize all HTTP proxy port data structures needed to accept connections.
 */
void init_accept_L4RProxyServer(int n_accept_threads = 0);

/** Checkes whether we can call start_HttpProxyServer().
 */
void init_L4RProxyServer(EThread *);

/** Start the proxy server.
    The port data should have been created by @c prep_HttpProxyServer().
*/
void start_L4RProxyServer();

void stop_L4RProxyServer();

void start_L4RProxyServerBackDoor(int port, int accept_threads = 0);
