...
//  state machines
DList(L4RSM, debug_link) debug_sm_list;
...
L4RVCTable::cleanup_entry(L4RVCTableEntry *e)
{
  ...
  if (e->in_tunnel == false) {
    ...
    switch (e->vc_type) {
    case HTTP_UA_VC:
      // proxy.process.http.current_client_transactions is decremented in L4RSM::destroy
      break;
    default:
      // This covers:
...
L4RSM::state_add_to_list(int event, void * /* data ATS_UNUSED */)
{
  ...
  if (t_state.http_config_param->enable_http_info) {
    ...
    MUTEX_TRY_LOCK(lock, L4RSMList[bucket].mutex, mutex->thread_holding);
    ...
    if (lock.is_locked()) {
      L4RSMList[bucket].sm_list.push(this);
    }
  }
...
L4RSM::state_remove_from_list(int event, void * /* data ATS_UNUSED */)
{
  ...
  if (t_state.http_config_param->enable_http_info) {
    ...
    L4RSMList[bucket].sm_list.remove(this);
  }
...
void
L4RSM::setup_client_read_request_header()
{
  ink_assert(ua_entry->vc_handler == &L4RSM::state_read_client_request_header);
...
{
  STATE_ENTER(&L4RSM::state_read_client_request_header, event);
...
L4RSM::wait_for_full_body()
{
  ...
  L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::tunnel_handler_post);
  ...
  p = tunnel.add_producer(ua_entry->vc, post_bytes, buf_start, &L4RSM::tunnel_handler_post_ua, HT_BUFFER_READ, "ua post buffer");
  if (chunked) {
...
void
L4RSM::setup_push_read_response_header()
{
  ink_assert(server_session == nullptr);
  ink_assert(server_entry == nullptr);
  ...
  ua_entry->vc_handler = &L4RSM::state_read_push_response_header;
...
int
L4RSM::state_read_push_response_header(int event, void *data)
{
  STATE_ENTER(&L4RSM::state_read_push_response_header, event);
  ink_assert(ua_entry->read_vio == (VIO *)data);
...
// int L4RSM::state_request_wait_for_transform_read(int event, void* data)
//
...
//
int
L4RSM::state_request_wait_for_transform_read(int event, void *data)
{
  STATE_ENTER(&L4RSM::state_request_wait_for_transform_read, event);
  int64_t size;
  ...
  // FALLTHROUGH
  default:
    state_common_wait_for_transform_read(&post_transform_info, &L4RSM::tunnel_handler_post, event, data);
    break;
  }
...
// int L4RSM::state_response_wait_for_transform_read(int event, void* data)
//
//   We've done a successful transform open and issued a do_io_write
//    to the transform.  We are now ready for the transform  to tell
//    us it is now ready to be read from and it done modifying the
...
//
int
L4RSM::state_response_wait_for_transform_read(int event, void *data)
{
  STATE_ENTER(&L4RSM::state_response_wait_for_transform_read, event);
  ...
  case TRANSFORM_READ_READY:
    ...
    break;
  default:
    state_common_wait_for_transform_read(&transform_info, &L4RSM::tunnel_handler, event, data);
    break;
  }
...
// int L4RSM::state_common_wait_for_transform_read(...)
//
//   This function handles the overlapping cases between request and response
//     transforms which prevents code duplication
//
int
L4RSM::state_common_wait_for_transform_read(HttpTransformInfo *t_info, L4RSMHandler tunnel_handler, int event, void *data)
{
  STATE_ENTER(&L4RSM::state_common_wait_for_transform_read, event);
...
int
L4RSM::state_read_server_response_header(int event, void *data)
{
  STATE_ENTER(&L4RSM::state_read_server_response_header, event);
...
//////////////////////////////////////////////////////////////////////////
//
//  L4RSM::state_handle_stat_page()
//
//////////////////////////////////////////////////////////////////////////
int
L4RSM::state_handle_stat_page(int event, void *data)
{
  STATE_ENTER(&L4RSM::state_handle_stat_page, event);
  switch (event) {
...
//////////////////////////////////////////////////////////////////////////
int
L4RSM::state_cache_open_write(int event, void *data)
{
  STATE_ENTER(&L4RSM : state_cache_open_write, event);
...
inline void
L4RSM::setup_cache_lookup_complete_api()
{
...
//////////////////////////////////////////////////////////////////////////
int
L4RSM::state_cache_open_read(int event, void *data)
{
  STATE_ENTER(&L4RSM::state_cache_open_read, event);
...
// void L4RSM::tunnel_handler_post_or_put()
//
//   Handles the common cleanup tasks for Http post/put
//   to prevent code duplication
//
void
L4RSM::tunnel_handler_post_or_put(HttpTunnelProducer *p)
{
...
// int L4RSM::tunnel_handler_post(int event, void* data)
//
//   Handles completion of any http request body tunnel
//     Having 'post' in its name is a misnomer
//
int
L4RSM::tunnel_handler_post(int event, void *data)
{
  STATE_ENTER(&L4RSM::tunnel_handler_post, event);
...
int
L4RSM::tunnel_handler_cache_fill(int event, void *data)
{
  STATE_ENTER(&L4RSM::tunnel_handler_cache_fill, event);
...
int
L4RSM::tunnel_handler_100_continue(int event, void *data)
{
  STATE_ENTER(&L4RSM::tunnel_handler_100_continue, event);
...
int
L4RSM::tunnel_handler_push(int event, void *data)
{
  STATE_ENTER(&L4RSM::tunnel_handler_push, event);
...
int
L4RSM::tunnel_handler_server(int event, HttpTunnelProducer *p)
{
  STATE_ENTER(&L4RSM::tunnel_handler_server, event);
  ...
  case VC_EVENT_EOS:
    ...
    if (is_http_server_eos_truncation(p)) {
      SMDebug("http", "[%" PRId64 "] [L4RSM::tunnel_handler_server] aborting HTTP tunnel due to server truncation", sm_id);
      ...
      SMDebug("http", "[%" PRId64 "] [L4RSM::tunnel_handler_server] finishing HTTP tunnel", sm_id);
      p->read_success               = true;
...
// int L4RSM::tunnel_handler_100_continue_ua(int event, HttpTunnelConsumer* c)
//
...
//
int
L4RSM::tunnel_handler_100_continue_ua(int event, HttpTunnelConsumer *c)
{
  STATE_ENTER(&L4RSM::tunnel_handler_100_continue_ua, event);
...
int
L4RSM::tunnel_handler_ua(int event, HttpTunnelConsumer *c)
{
  bool close_connection     = true;
  HttpTunnelProducer *p     = nullptr;
  HttpTunnelConsumer *selfc = nullptr;

  STATE_ENTER(&L4RSM::tunnel_handler_ua, event);
  ink_assert(c->vc == ua_txn);
  milestones[TS_MILESTONE_UA_CLOSE] = Thread::get_hrtime();
  ...
  // to a source in L4RSM::tunnel_handler_transform_write
  //
...
int
L4RSM::tunnel_handler_ua_push(int event, HttpTunnelProducer *p)
{
  STATE_ENTER(&L4RSM::tunnel_handler_ua_push, event);
...
int
L4RSM::tunnel_handler_cache_read(int event, HttpTunnelProducer *p)
{
  STATE_ENTER(&L4RSM::tunnel_handler_cache_read, event);
...
int
L4RSM::tunnel_handler_cache_write(int event, HttpTunnelConsumer *c)
{
  STATE_ENTER(&L4RSM::tunnel_handler_cache_write, event);
...
int
L4RSM::tunnel_handler_post_ua(int event, HttpTunnelProducer *p)
{
  STATE_ENTER(&L4RSM::tunnel_handler_post_ua, event);
  ...
  case VC_EVENT_ERROR:
    ...
    lsm_release_assert(ua_entry->in_tunnel == true);
    if (p->consumer_list.head && p->consumer_list.head->vc_type == HT_TRANSFORM) {
      lsm_release_assert(post_transform_info.entry->in_tunnel == true);
    } else if (server_entry != nullptr) {
      lsm_release_assert(server_entry->in_tunnel == true);
    }
    break;
...
int
L4RSM::tunnel_handler_for_partial_post(int event, void * /* data ATS_UNUSED */)
{
  STATE_ENTER(&L4RSM::tunnel_handler_for_partial_post, event);
  tunnel.deallocate_buffers();
  tunnel.reset();
...
int
L4RSM::tunnel_handler_post_server(int event, HttpTunnelConsumer *c)
{
  STATE_ENTER(&L4RSM::tunnel_handler_post_server, event);
...
int
L4RSM::tunnel_handler_ssl_producer(int event, HttpTunnelProducer *p)
{
...
int
L4RSM::tunnel_handler_ssl_consumer(int event, HttpTunnelConsumer *c)
{
...
int
L4RSM::tunnel_handler_transform_write(int event, HttpTunnelConsumer *c)
{
  STATE_ENTER(&L4RSM::tunnel_handler_transform_write, event);
  ...
  // is instead handled in L4RSM::tunnel_handler_ua
  //
...
int
L4RSM::tunnel_handler_transform_read(int event, HttpTunnelProducer *p)
{
  STATE_ENTER(&L4RSM::tunnel_handler_transform_read, event);
...
int
L4RSM::tunnel_handler_plugin_agent(int event, HttpTunnelConsumer *c)
{
  STATE_ENTER(&L4RSM::tunnel_handler_plugin_client, event);
...
int
L4RSM::state_remap_request(int event, void * /* data ATS_UNUSED */)
{
  STATE_ENTER(&L4RSM::state_remap_request, event);
...
void
L4RSM::do_remap_request(bool run_inline)
{
  SMDebug("http_seq", "[L4RSM::do_remap_request] Remapping request");
 ...
 */
void
L4RSM::parse_range_and_compare(MIMEField *field, int64_t content_length)
{
...
void
L4RSM::calculate_output_cl(int64_t num_chars_for_ct, int64_t num_chars_for_cl)
{
  int i;
...
void
L4RSM::do_range_parse(MIMEField *range_field)
{
...
// HttpTunnel
void
L4RSM::do_range_setup_if_necessary()
{
  MIMEField *field;
...
void
L4RSM::do_cache_lookup_and_read()
{
  // TODO decide whether to uncomment after finish testing redirect
  ...
  SMDebug("http_seq", "[L4RSM::do_cache_lookup_and_read] [%" PRId64 "] Issuing cache lookup for URL %s", sm_id,
...
void
L4RSM::do_cache_delete_all_alts(Continuation *cont)
{
  // Do not delete a non-existant object.
  ink_assert(t_state.cache_info.object_read);

  SMDebug("http_seq", "[L4RSM::do_cache_delete_all_alts] Issuing cache delete for %s",
...
inline void
L4RSM::do_cache_prepare_write_transform()
{
...
L4RSM::do_cache_prepare_update()
{
  if (t_state.cache_info.object_read != nullptr && t_state.cache_info.object_read->valid() &&
    ...
    L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::state_cache_open_write);
...
void
L4RSM::do_cache_prepare_action(HttpCacheSM *c_sm, CacheHTTPInfo *object_read_info, bool retry, bool allow_multiple)
{
...
void
L4RSM::send_origin_throttled_response()
{
...
// void L4RSM::handle_post_failure()
//
...
void
L4RSM::handle_post_failure()
{
  STATE_ENTER(&L4RSM::handle_post_failure, VC_EVENT_NONE);
...
void
L4RSM::setup_transform_to_server_transfer()
{
  ink_assert(post_transform_info.vc != nullptr);
  ...
  HttpTunnelProducer *p = tunnel.add_producer(post_transform_info.vc, nbytes, buf_start, &L4RSM::tunnel_handler_transform_read,
                                              HT_TRANSFORM, "post transform");
  tunnel.chain(c, p);
  post_transform_info.entry->in_tunnel = true;

  tunnel.add_consumer(server_entry->vc, post_transform_info.vc, &L4RSM::tunnel_handler_post_server, HT_HTTP_SERVER,
                      "http server post");
  server_entry->in_tunnel = true;
...
void
L4RSM::do_setup_post_tunnel(HttpVC_t to_vc_type)
{
  bool chunked       = (t_state.client_info.transfer_encoding == HttpTransact::CHUNKED_ENCODING);
  ...
  if (is_using_post_buffer ||
    ...
    ua_buffer_reader->consume(client_request_body_bytes);
    p = tunnel.add_producer(ua_entry->vc, post_bytes - transfered_bytes, buf_start, &L4RSM::tunnel_handler_post_ua, HT_HTTP_CLIENT,
                            "user agent post");
  }
  ua_entry->in_tunnel = true;
  ...
  case HTTP_TRANSFORM_VC:
    L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::state_request_wait_for_transform_read);
    ink_assert(post_transform_info.entry != nullptr);
    ink_assert(post_transform_info.entry->vc == post_transform_info.vc);
    tunnel.add_consumer(post_transform_info.entry->vc, ua_entry->vc, &L4RSM::tunnel_handler_transform_write, HT_TRANSFORM,
                        "post transform");
    post_transform_info.entry->in_tunnel = true;
    break;
  case HTTP_SERVER_VC:
    ...
    if (post_redirect) {
      L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::tunnel_handler_for_partial_post);
      tunnel.add_consumer(server_entry->vc, HTTP_TUNNEL_STATIC_PRODUCER, &L4RSM::tunnel_handler_post_server, HT_HTTP_SERVER,
                          "redirect http server post");
    } else {
      L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::tunnel_handler_post);
      tunnel.add_consumer(server_entry->vc, ua_entry->vc, &L4RSM::tunnel_handler_post_server, HT_HTTP_SERVER, "http server post");
    }
    server_entry->in_tunnel = true;
    break;
  default:
...
// void L4RSM::perform_transform_cache_write_action()
//
//   Called to do cache write from the transform
//
void
L4RSM::perform_transform_cache_write_action()
{
...
// void L4RSM::perform_cache_write_action()
//
...
//
void
L4RSM::perform_cache_write_action()
{
...
int
L4RSM::write_header_into_buffer(HTTPHdr *h, MIOBuffer *b)
{
  int bufindex;
  int dumpoffset;
  int done, tmp;
...
void
L4RSM::setup_server_send_request_api()
{
...
void
L4RSM::setup_server_send_request()
{
  int hdr_length;
  ...
  lsm_release_assert(server_entry != nullptr);
  lsm_release_assert(server_session != nullptr);
  lsm_release_assert(server_entry->vc == server_session);
...
void
L4RSM::setup_server_read_response_header()
{
  ink_assert(server_session != nullptr);
  ink_assert(server_entry != nullptr);
  ...
  server_entry->vc_handler = &L4RSM::state_read_server_response_header;
...
HttpTunnelProducer *
L4RSM::setup_cache_read_transfer()
{
  ...
  HttpTunnelProducer *p = tunnel.add_producer(cache_sm.cache_read_vc, doc_size, buf_start, &L4RSM::tunnel_handler_cache_read,
                                              HT_CACHE_READ, "cache read");
  tunnel.add_consumer(ua_entry->vc, cache_sm.cache_read_vc, &L4RSM::tunnel_handler_ua, HT_HTTP_CLIENT, "user agent");
...
HttpTunnelProducer *
L4RSM::setup_cache_transfer_to_transform()
{
  int64_t alloc_index;
  int64_t doc_size;
  ...
  L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::state_response_wait_for_transform_read);

  HttpTunnelProducer *p = tunnel.add_producer(cache_sm.cache_read_vc, doc_size, buf_start, &L4RSM::tunnel_handler_cache_read,
                                              HT_CACHE_READ, "cache read");

  tunnel.add_consumer(transform_info.vc, cache_sm.cache_read_vc, &L4RSM::tunnel_handler_transform_write, HT_TRANSFORM,
                      "transform write");
...
void
L4RSM::setup_cache_write_transfer(HttpCacheSM *c_sm, VConnection *source_vc, HTTPInfo *store_info, int64_t skip_bytes,
                                   const char *name)
{
  ...
  tunnel.add_consumer(c_sm->cache_write_vc, source_vc, &L4RSM::tunnel_handler_cache_write, HT_CACHE_WRITE, name, skip_bytes);

  c_sm->cache_write_vc = nullptr;
}

void
L4RSM::setup_100_continue_transfer()
{
  ...
  L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::tunnel_handler_100_continue);
  ...
  tunnel.add_consumer(ua_entry->vc, HTTP_TUNNEL_STATIC_PRODUCER, &L4RSM::tunnel_handler_100_continue_ua, HT_HTTP_CLIENT,
                      "user agent");
...
HttpTunnelProducer *
L4RSM::setup_server_transfer_to_transform()
{
  int64_t alloc_index;
  int64_t nbytes;
  ...
  HttpTunnelProducer *p =
    tunnel.add_producer(server_entry->vc, nbytes, buf_start, &L4RSM::tunnel_handler_server, HT_HTTP_SERVER, "http server");

  tunnel.add_consumer(transform_info.vc, server_entry->vc, &L4RSM::tunnel_handler_transform_write, HT_TRANSFORM,
                      "transform write");
...
HttpTunnelProducer *
L4RSM::setup_transfer_from_transform()
{
  ...
  HttpTunnelProducer *p = tunnel.add_producer(transform_info.vc, INT64_MAX, buf_start, &L4RSM::tunnel_handler_transform_read,
                                              HT_TRANSFORM, "transform read");
  tunnel.chain(c, p);

  tunnel.add_consumer(ua_entry->vc, transform_info.vc, &L4RSM::tunnel_handler_ua, HT_HTTP_CLIENT, "user agent");
...
HttpTunnelProducer *
L4RSM::setup_transfer_from_transform_to_cache_only()
{
  int64_t alloc_index       = find_server_buffer_size();
  MIOBuffer *buf            = new_MIOBuffer(alloc_index);
  IOBufferReader *buf_start = buf->alloc_reader();
...
void
L4RSM::setup_server_transfer_to_cache_only()
{
...
L4RSM::setup_server_transfer()
{
  ...
  tunnel.add_consumer(ua_entry->vc, server_entry->vc, &L4RSM::tunnel_handler_ua, HT_HTTP_CLIENT, "user agent");
...
HttpTunnelProducer *
L4RSM::setup_push_transfer_to_cache()
{
  ...
  L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::tunnel_handler_push);

  HttpTunnelProducer *p =
    tunnel.add_producer(ua_entry->vc, nbytes, buf_start, &L4RSM::tunnel_handler_ua_push, HT_HTTP_CLIENT, "user_agent");
...
L4RSM::setup_blind_tunnel(bool send_response_hdr, IOBufferReader *initial)
{
  ...
  p_os =
    tunnel.add_producer(server_entry->vc, -1, r_to, &L4RSM::tunnel_handler_ssl_producer, HT_HTTP_SERVER, "http server - tunnel");

  c_ua = tunnel.add_consumer(ua_entry->vc, server_entry->vc, &L4RSM::tunnel_handler_ssl_consumer, HT_HTTP_CLIENT,
                             "user agent - tunnel");

  p_ua = tunnel.add_producer(ua_entry->vc, -1, r_from, &L4RSM::tunnel_handler_ssl_producer, HT_HTTP_CLIENT, "user agent - tunnel");

  c_os = tunnel.add_consumer(server_entry->vc, ua_entry->vc, &L4RSM::tunnel_handler_ssl_consumer, HT_HTTP_SERVER,
                             "http server - tunnel");
...
void
L4RSM::setup_plugin_agents(HttpTunnelProducer *p)
{
  ...
  while (agent) {
    ...
    tunnel.add_consumer(contp, p->vc, &L4RSM::tunnel_handler_plugin_agent, HT_HTTP_CLIENT, "plugin agent");
...
inline void
L4RSM::transform_cleanup(TSHttpHookID hook, HttpTransformInfo *info)
{
...
void
L4RSM::dump_state_hdr(HTTPHdr *h, const char *s)
{
...
L4RSM::set_next_state()
{
  ...
  case HttpTransact::SM_ACTION_REMAP_REQUEST: {
    if (!remapProcessor.using_separate_thread()) {
      ...
      L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::state_remap_request);
  ...
  case HttpTransact::SM_ACTION_CACHE_LOOKUP: {
    L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::state_cache_open_read);
  ...
  case HttpTransact::SM_ACTION_INTERNAL_REQUEST: {
    L4R_SM_SET_DEFAULT_HANDLER(&L4RSM::state_handle_stat_page);
...
L4RSM::do_redirect()
{
  SMDebug("http_redirect", "[L4RSM::do_redirect]");
  if (!enable_redirection || redirection_tries > t_state.txn_conf->number_of_redirections) {
    this->postbuf_clear();
    return;
  }
...
void
L4RSM::redirect_request(const char *redirect_url, const int redirect_len)
{
  SMDebug("http_redirect", "[L4RSM::redirect_request]");
  ...
  if (t_state.hdr_info.client_request.presence(MIME_PRESENCE_HOST)) {
    ...
    if (host != nullptr) {
      ...
      if (redirectScheme == nullptr) {
        ...
        SMDebug("http_redirect", "[L4RSM::redirect_request] URL without scheme %.*s", redirectSchemeLen, redirectScheme);
      }
      ...
      if (valid_origHost) {
        ...
        // the client request didn't have a host, so use the current origin host
        SMDebug("http_redirect", "[L4RSM::redirect_request] keeping client request host %s://%s", next_hop_scheme, origHost);
