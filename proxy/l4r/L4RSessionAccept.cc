...
bool
L4RSessionAccept::accept(NetVConnection *netvc, MIOBuffer *iobuf, IOBufferReader *reader)
{
  ...
  if (is_debug_tag_set("http_seq")) {
    Debug("http_seq", "[L4RSessionAccept:mainEvent %p] accepted connection from %s transport type = %d", netvc,
...
int
L4RSessionAccept::mainEvent(int event, void *data)
{
