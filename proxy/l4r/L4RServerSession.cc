...
ClassAllocator<L4RServerSession> l4rServerSessionAllocator("l4rServerSessionAllocator");

void
L4RServerSession::destroy()
{
...
void
L4RServerSession::new_connection(NetVConnection *new_vc)
{
...
VIO *
L4RServerSession::do_io_read(Continuation *c, int64_t nbytes, MIOBuffer *buf)
{
  return server_vc->do_io_read(c, nbytes, buf);
}

VIO *
L4RServerSession::do_io_write(Continuation *c, int64_t nbytes, IOBufferReader *buf, bool owner)
{
  return server_vc->do_io_write(c, nbytes, buf, owner);
}

void
L4RServerSession::do_io_shutdown(ShutdownHowTo_t howto)
{
...
L4RServerSession::do_io_close(int alerrno)
{
  ...
  Debug("l4r_ss", "[%" PRId64 "] session closing, netvc %p", con_id, server_vc);
  ...
  //    ip_port_text_buffer addrbuf;
  //    Debug("l4r_ss", "[%" PRId64 "] connection closed, ip: %s, count: %u", con_id,
  //          ats_ip_nptop(&get_server_ip().sa, addrbuf, sizeof(addrbuf)),
  //          connection_count->getCount(get_server_ip(), hostname_hash, sharing_match));
  //  } else {
...
void
L4RServerSession::reenable(VIO *vio)
{
  server_vc->reenable(vio);
}

// void L4RServerSession::release()
//
//   Releases the session for K-A reuse
//
void
L4RServerSession::release()
{
