...
#define L4rSsnDebug(fmt, ...) SsnDebug(this, "l4r_cs", fmt, __VA_ARGS__)
...
//  client sessions
DList(L4RClientSession, debug_link) debug_cs_list2;
ink_mutex debug_cs_list2_mutex;

#endif /* USE_HTTP_DEBUG_LISTS */

ClassAllocator<L4RClientSession> l4rClientSessionAllocator("l4rClientSessionAllocator");
...
L4RClientSession::destroy()
{
  ...
  if (!in_destroy) {
    ...
    L4rSsnDebug("[%" PRId64 "] session destroy", con_id);
    ink_release_assert(!client_vc);
    ink_assert(read_buffer);
    ink_release_assert(transact_count == released_transactions);
    do_api_callout(TS_HTTP_SSN_CLOSE_HOOK);
  } else {
    Warning("l4r: Attempt to double ssn close");
  }
}

void
L4RClientSession::release_transaction()
{
...
L4RClientSession::free()
{
...
#ifdef USE_HTTP_DEBUG_LISTS
  ink_mutex_acquire(&debug_cs_list2_mutex);
  debug_cs_list2.remove(this);
  ink_mutex_release(&debug_cs_list2_mutex);
#endif
  ...
  super::free();
  THREAD_FREE(this, l4rClientSessionAllocator, this_thread());
}

void
L4RClientSession::new_connection(NetVConnection *new_vc, MIOBuffer *iobuf, IOBufferReader *reader, bool backdoor)
{
...
#ifdef USE_HTTP_DEBUG_LISTS
  ink_mutex_acquire(&debug_cs_list2_mutex);
  debug_cs_list2.push(this);
  ink_mutex_release(&debug_cs_list2_mutex);
#endif

  L4rSsnDebug("[%" PRId64 "] session born, netvc %p", con_id, new_vc);

  client_vc->set_tcp_congestion_control(CLIENT_SIDE);

  read_buffer = iobuf ? iobuf : new_MIOBuffer(/*HTTP_HEADER_BUFFER_SIZE_INDEX*/BUFFER_SIZE_INDEX_4K);
...
VIO *
L4RClientSession::do_io_read(Continuation *c, int64_t nbytes, MIOBuffer *buf)
{
  return client_vc->do_io_read(c, nbytes, buf);
}

VIO *
L4RClientSession::do_io_write(Continuation *c, int64_t nbytes, IOBufferReader *buf, bool owner)
{
  /* conditionally set the tcp initial congestion window
     before our first write. */
  L4rSsnDebug("tcp_init_cwnd_set %d", (int)tcp_init_cwnd_set);
...
void
L4RClientSession::set_tcp_init_cwnd()
{
  //if (!trans.get_sm()) {
  //  return;
  //}
  ...
  L4rSsnDebug("desired TCP congestion window is %d", desired_tcp_init_cwnd);
  if (desired_tcp_init_cwnd == 0) {
    return;
  }
  if (get_netvc()->set_tcp_init_cwnd(desired_tcp_init_cwnd) != 0) {
    L4rSsnDebug("set_tcp_init_cwnd(%d) failed", desired_tcp_init_cwnd);
  }
}

void
L4RClientSession::do_io_shutdown(ShutdownHowTo_t howto)
{
  client_vc->do_io_shutdown(howto);
}

void
L4RClientSession::do_io_close(int alerrno)
{
  if (read_state == HCS_CLOSED) {
  ...
  //  SET_HANDLER(&L4RClientSession::state_wait_for_close);
  //  L4rSsnDebug("[%" PRId64 "] session half close", con_id);
  ...
  //} else {
  {
    ...
    L4rSsnDebug("[%" PRId64 "] session closed", con_id);
...
int
L4RClientSession::state_wait_for_close(int event, void *data)
{
  STATE_ENTER(&L4RClientSession::state_wait_for_close, event, data);
...
int
L4RClientSession::state_slave_keep_alive(int event, void *data)
{
  STATE_ENTER(&L4RClientSession::state_slave_keep_alive, event, data);
...
int
L4RClientSession::state_keep_alive(int event, void *data)
{
  ...
  STATE_ENTER(&L4RClientSession::state_keep_alive, event, data);
...
void
L4RClientSession::reenable(VIO *vio)
{
...
void
L4RClientSession::release(ProxyClientTransaction *trans)
{
#if 0
  ...
  if (more_to_read) {
    ...
    L4rSsnDebug("[%" PRId64 "] data already in buffer, starting new transaction", con_id);
    new_transaction();
  } else {
    L4rSsnDebug("[%" PRId64 "] initiating io for next header", con_id);
    trans->set_restart_immediate(false);
    read_state = HCS_KEEP_ALIVE;
    SET_HANDLER(&L4RClientSession::state_keep_alive);
...
void
L4RClientSession::new_transaction()
{
